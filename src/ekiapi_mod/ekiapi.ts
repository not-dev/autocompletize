...
const getEkiList = (apiKey: string, target:HTMLInputElement, callback:({ target, data }:Response) => void) => {
    ...
    if (apiKey) {
      ...
      axios.get(url, data)
        .then(res => {
          ...
          const args = { target: target, data: ekiNames }
